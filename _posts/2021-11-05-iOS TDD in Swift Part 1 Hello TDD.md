...
Test-driven development(后边简称 TDD) 正好能解决上边两个现象，那么它是如何避免的呢，本篇来讲一下 TDD 和循环，用一个小的实践来演示 TDD 开发。
...
TDD 是确保你的代码能执行成功和将来执行依然成功最好的方式。你可能认为不使用 TDD，不写 UT，你也能写出能够上线的代码。你当然可以，短期很容易做到，但是长期下来，如果逻辑逐渐复杂，修改 bug 要阅读的代码消耗的时间将指数增长，当然大环境都是项目排期紧，很难有时间写 UT，最终还是看个人追求。

关于 test，并非必须使用 TDD，你当然可以完成业务开发后再写 UT，或者你可以直接手动测试你的代码。

为什么 TDD 比其他的方式好？

好的 test 能保证你的 App 按照期望运行，然而不是所有的 Test 都是"好的"。好的 test 应该是可失败的、可重复的、快速运行的和可维护的。
